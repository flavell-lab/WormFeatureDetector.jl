...
"""
function find_gut_granules(path::String, names::Array{String, 1}, threshold, density, radius; mhd="MHD", out="")
    """ Finds location of gut granules. """
    gut_imgs = []
    @showprogress for name in names
        m = MHD(joinpath(path, mhd, name*".mhd"))
        spacing = map(x->parse(Float64,x), split(m.mhd_spec_dict["ElementSpacing"]))
        size = map(x->parse(Int16,x), split(m.mhd_spec_dict["DimSize"]))
        img = read_img(m)
        gut_img = 0x0001 .- find_gut_granules_img(img, threshold, density, radius)
        if out != ""
            out_path = joinpath(path, out)
            create_dir(out_path)
            write_MHD_spec(joinpath(out_path, name*".mhd"), spacing[1], spacing[3], size[1], size[2], size[3], name*".raw")
            write_raw(joinpath(out_path, name*".raw"), gut_img)
        end
...
function find_hsn(path::String, frames, mhd::String, img_prefix::String, channel::Integer, threshold_outer::Real, density_outer::Real, radius_outer, 
    ...
    @showprogress for i=1:n
        frame = frames[i]
        img = read_mhd(path, img_prefix, mhd, frame, channel)
        sx, sy, sz = size(img)
